...
_Last reviewed: 2026-09-24 (branch `feat/observability-detection`, on top of HEAD 647f81e) — **agent-observability Phase 1 review fixups**: (1) added `analysis.DiscoverAgentObservabilitySignals`, wired via a new `analysis.MergeObservabilitySignals` alongside the existing file-level pass — it reads `AgentDef.Kwargs` for the three SDKs whose "traces are on" evidence lives on the agent constructor rather than a standalone init call (Pydantic AI `instrument=True`, Vercel `experimental_telemetry`, LangChain `callbacks=`) and emits the `instrument_kwarg` signal `repo_observability_initialized`'s own doc comment already claimed to honor but that nothing previously produced — closing a false positive where an instrumented-via-kwarg-only Pydantic AI project misfired OBS-001; (2) gated the Python bare (receiver-less) init-callee names `init_logger` / `autolog` on the corresponding vendor import, mirroring the TS side's existing import gate — closes a false-negative where a project's own unrelated `init_logger()` helper silently suppressed all nine absence rules; (3) `OBS-002` / `OBS-003` now set `language: python` — their exporter-construction and content-capture detection (`pyObservabilitySignals`) has no TS/JS counterpart yet, so the rules no longer claim TS/JS coverage (via `vercel_ai` etc. in `applies_to`) that does not exist; (4) removed the half-wired `helicone` vendor (the `detectObsDeps` needle and `models.VendorHelicone`) — recon could detect the dependency but no AST pass ever produced a code signal for it, so any real Helicone user was permanently and unfixably flagged by OBS-005 as "declared but never wired." Deferred to whichever phase implements the base-URL-literal detection the design doc sketches. No schema/predicate/`schema_version` change — all four are discovery-logic and rule-metadata fixes on top of the Phase 1 rules landed below. Prior review 2026-09-21 (HEAD fbc5de0) — **agent-observability detection (Phase 1, static wiring)**: new inventory pass `analysis.DiscoverObservability` emits typed `ObservabilitySignal` records (vendor × Kind, where Kind grades evidence as import / init / exporter / content_capture / instrument_kwarg) across Python and TypeScript/JavaScript only — Go, C#, PHP and Rust are NOT inspected, and the new `repo_observability_inspectable` predicate is what stops an absence rule firing there on an empty result. Recon gains a separate `RepoProfile.ObsDeps` needle scan (deliberately not folded into `SDKDeps`, which would make META-002 fire forever on any repo declaring langfuse). Seven new repo-scope predicates; `repo_has_observability` is the catalog's first **dual-scope** predicate (repo + agent), which required relaxing `TestPredicatesByScope_MirrorsStruct` to a named `dualScopePredicates` allow-list. 16 new rules (`schema_version` 17 → 18): nine repo-scope absence rules at severity low (OAI-203, CSDK-208, LC-202, PYD-202, VAI-201, CREW-202, ADK-202, AG2-202, MCP-202), three agent-scope outlier rules gated on the repo already being instrumented (PYD-107, LC-112, VAI-101), and a new `observability` category holding cross-SDK quality rules OBS-001 (imported but never initialized, medium), OBS-002 (console-only exporter, low) and OBS-003 (full prompt/response capture, medium) and OBS-005 (an observability package declared in a hand-edited manifest that no code imports, medium — the one predicate reading recon's `RepoProfile.ObsDeps` rather than code signals, with `poetry.lock` excluded because a transitive entry proves nobody's intent; the nine absence rules gained `repo_observability_declared: false` so "shipped a tracing dep and never wired it" is medium while "never tried" stays low). The `observability` category loads unconditionally in `LoadFor` like `openshell`/`claude_skill` — it is not an SDK, so SDK-gating would drop the pack entirely; that gap was caught by an end-to-end CLI run, not by the per-rule tests. Runtime trace ingestion is Phase 2 and is NOT implemented. Rule counts re-derived: 292 fixture / 292 production. Prior review 2026-09-21 (pending commit) — outreach-feedback investigation
(Aug–Sep 2026 batch) fixed two confirmed rule bugs. **CSKILL-080/081**
(claude_skill/skill_quality_text.yaml) moved from raw-substring
`skill_*_has_text` matching onto a new sentence-scoped, word-boundary
predicate, `skill_text_matches` (`schema_version` 17 → 18), closing confirmed
false positives: a skill documenting "API keys live in GitHub Actions
secrets" (named a data class, didn't handle it — closed via `require_context`
requiring an operational verb in the same sentence) and a skill listing "hash
a string" as a worked example (closed via `exclude_context`). CSKILL-082..087
still use the raw-substring predicates unmodified — no confirmed FP against
them yet. Separately, **CSDK-201/202** (bypassPermissions) were reported as
under-scoped for not checking `allowed_tools` at the same site; that premise
was refuted against the Agent SDK's own docs (`allowed_tools` only
auto-approves, it does not restrict — an empty/narrow allow-list next to
`bypassPermissions` is the *most* dangerous shape, not a mitigated one — see
`docs/decisions/tool-allowlist-scope.md`), and the fix was retargeted onto
the two mechanisms that genuinely do restrict: **CSDK-202** now uses a new
per-construction-site correlation predicate,
`repo_claude_options_mode_without_kwarg`, firing only when a
`bypassPermissions` `ClaudeAgentOptions(...)` lacks `disallowed_tools` at
that same site (fixing a repo-wide-vs-per-site gap in the process); new rule
**CSDK-206** (medium/0.6) fires the complementary case, `bypassPermissions`
paired with a deny-list that may still leave a broad surface. **CSDK-103**
(Python) / **CSDK-120** (TypeScript) `AgentDefinition` rules gained a
co-check on `tools` — sound here because, unlike `allowed_tools`,
`AgentDefinition.tools` genuinely restricts the agent — going silent on a
genuinely read-only tools list, rules-only, no schema change. Rule count:
**277** (was 210 as of the last figure recorded in `CLAUDE.md`, which itself
predates a large unrelated rules-repo pull that added `autogen`, `network`,
`observability`, and `tool_definition` packs across most SDK categories —
re-derive, don't trust a cached figure, per `grep -rhoE
'^\s*-\s*id:\s*\S+' testdata/rules-fixture/*/*.yaml | wc -l`). Prior review 2026-09-15 (HEAD 21a806f) — new rules **OAI-118** (Python) and **OAI-119** (TypeScript): an OpenAI Agents SDK agent that wires an MCP server (`MCPServerStdio`/`MCPServerSse`/`MCPServerStreamableHttp`) with no `tool_filter`/`toolFilter` — the self-hosted sibling of OAI-115/116's `HostedMCPTool`/`hostedMcpTool` allow-list checks, both severity high / confidence 0.75. Required a discovery fix first: Python MCP server construction (`classifyMCPServerCall`, the `async with X() as srv:` alias path, and the alias-to-def copy in `ResolveEdges`) now captures constructor kwargs onto `MCPServerDef.Kwargs` — previously nil by design ("not captured at v1"); TS already captured them. New predicate `agent_mcp_server_kwarg_missing` (`schema_version` 15 → 16) takes a `classes:` list rather than one `class:` like `agent_hosted_tool_kwarg_present`, because all three MCP server classes share `tool_filter` as the same allow-list mechanism — one rule per language, not one per class. Closes the last of the three follow-ups in `docs/decisions/tool-allowlist-scope.md`. Prior review 2026-09-10 (HEAD 05bcd9e) — new rule **OAI-117**, a second and independent TypeScript sibling alongside OAI-116 on the same `hostedMcpTool({...})` construct: fires when `requireApproval` is omitted from the options, severity medium / confidence 0.65 (below OAI-115/116 — the mechanism is certain, verified directly in `openai-agents-js`'s `tool.ts` (both the `serverUrl` and `connectorId` branches inject `require_approval: 'never'` when the option is `undefined`), but the predicate cannot distinguish negligent omission from a deliberately read-only MCP server where `'never'` is the correct setting). Does not check `allowedTools` and is not combined with OAI-116 — an agent can set one control and miss the other, and a `hostedMcpTool(...)` missing both draws both findings. Fires on omission only, not an explicit `requireApproval: 'never'` (a reviewable choice OAI-116's own fix text recommends). Re-verified against `openai-agents-python` that Python has no equivalent gap (no SDK-level default injection in `tool.py`; the Responses API's `Mcp` TypedDict has no default either) — no parallel Python rule. Rules-only; no schema/predicate/`schema_version` change. Closes the `require_approval` / `requireApproval` follow-up left open by the prior OAI-116 review in `docs/decisions/tool-allowlist-scope.md`. Prior review 2026-09-09 (HEAD 1fb7d38) — new rule **OAI-116**, the TypeScript sibling of OAI-115: a `hostedMcpTool({...})` factory call (`@openai/agents`) with no `allowedTools` allow-list, severity high / confidence 0.75 (one notch above OAI-115's 0.7 — the JS SDK injects `require_approval: 'never'` when `requireApproval` is omitted, foreclosing the platform-default approval fallback that justifies OAI-115's discount). Rules-only, built on the TS hosted-tool kwarg capture from the prior review; no schema/predicate/`schema_version` change. Closes the OpenAI Agents SDK half of Class 1 in `docs/decisions/tool-allowlist-scope.md`. Prior review 2026-09-07 (HEAD d9c8dc6) — TS OpenAI hosted-tool factory calls (`hostedMcpTool({...})`, `webSearchTool({...})`, etc.) now capture their options-object kwargs the same way the Python `HostedMCPTool(tool_config={...})` path already did, and `ResolveEdges` materializes the discovery-built def (precise factory-call `Location` + `Kwargs`) instead of synthesizing a bare one at the agent's line. Discovery-only, no rule/schema change — unblocks the OpenAI Agents SDK half of Class 1 in `docs/decisions/tool-allowlist-scope.md`, which also now records the verified flat `hostedMcpTool` options shape (no `toolConfig` wrapper, unlike Python). TS ADK and Vercel AI hosted-tool refs are unchanged (still agent-line Location, no kwargs) — a documented follow-up, not done here. Prior review 2026-09-01 — new rule CSDK-205, Claude SDK repo-scope: flags a `ClaudeAgentOptions(...)` session that sets `permission_mode="acceptEdits"` with no `disallowed_tools` deny-list — Claude SDK's `allowed_tools` only auto-approves, it never restricts, so the risk signal is a missing deny-list paired with a permissive mode, not an empty allow-list. Discovery-free: kwargs were already captured generically, so this is a rules-layer-only change (`schema_version` 15). Closes the Claude SDK half of Class 1 in `docs/decisions/tool-allowlist-scope.md`; the OpenAI Agents SDK half remains unresearched. Prior review 2026-08-24 — docs catch-up for execution-limit rules that already ship: OAI-112 `Runner.run`/`run_sync`/`run_streamed` with no `max_turns`, PYD-106 `agent.run`/`run_sync`/`run_stream` with no `usage_limits` — both via `DiscoverAgentRunCalls` in `internal/analysis/agent_run_calls.go`; CSDK-204 no `ClaudeAgentOptions.max_turns`; CREW-110 `Agent` with no `max_iter`. TypeScript Claude row also lists CSDK-121..124, which the 2026-07-07 note already claimed. PYD-104 also shipped in that window: new `agent_file_url_force_download` predicate with FileUrl-family / `WebFetchTool` `force_download` discovery. Prior review 2026-08-10 — NVIDIA NeMo Agent Toolkit recognized at recon only — new `nvidia-nat` dep needle in `detectSDKDeps`, catching bracketed extras like `nvidia-nat[langchain]`; no discovery pass, no rules, and deliberately not wired into `scanner.depNameToSDK` — that would emit a permanent false META-002 with no discovery to ever satisfy it. Prior review 2026-07-07 — rules audit corrections — ADK shell-tool class rename support (`ExecuteBashTool` alongside `BashTool` in discovery + ADK-008/102/103) and the `url_context`/`load_web_page` instance forms in ADK-110; four new TypeScript Claude agent rules CSDK-121..124 (AgentDefinition tool grants — Bash / WebSearch / write built-ins / WebFetch), closing the unaudited TS AgentDefinition gap; harmonized network-timeout callee lists across the six Python packs (Session aliases, urllib both forms, aliased aiohttp); repo-hygiene rules CSDK-203/OAI-202/ADK-201 no longer python-gated so TS-only repos get them; loader now skips the reserved `mappings/` pack subtree. Prior review 2026-06-14 — LangGraph: raw `StateGraph` graph discovery — the imperative `StateGraph(...)` → `add_node` / `add_edge` → `compile()` builder is now discovered as one `AgentDef` (Class `StateGraph`, SDK `langchain`), closing the "hand-wired graph finds nothing" gap; discovery-only — `langchain_state_graph` rules, `ToolNode`/`bind_tools` tool-edge resolution, and the TS equivalent are pending. Prior review 2026-06-08 — Agent Skills: enriched SKILL.md discovery — body-fact + bundled-file inventory, plus bundled-file content-scanning for egress/credential reads and committed secret literals, and hidden-Unicode detection (Tags-block U+E0000–E007F + bidi overrides) — and the `claude_skill` CSKILL-* rule pack (CSKILL-001/002/003/010/011/020/030/040/050/060 — 060 = description-vs-tools mismatch; `schema_version` 12; mapped to the OWASP Agentic Skills Top 10 / ASI in the rulebook) at the `skill` scope; plus a deterministic repo-wide **dependency BOM** (`ScanResult.Dependencies` parsed from declared manifests across all six languages — pip/npm/Go/Composer/NuGet/Cargo; optional CycloneDX `--bom-out`; pure inventory by default, TR-278, supersedes the skill-only TR-221) **plus an opt-in `--vuln-scan` layer that matches the repo's concretely-pinned deps against a pinned OSV snapshot and emits CVE/GHSA findings with CVSS-derived severity + fixed version (`internal/vulndb`; `trustabl vulndb pull` pre-warms the cache; snapshot version folded into `ScanID` only when enabled; TR-271)**; prior 2026-06-07 review covered JavaScript `.js`/`.jsx`/`.mjs`/`.cjs` via the shared TypeScript-family pipeline and Go/C#/PHP/Rust MCP discovery with field-based rules MCP-015..022)._

> **Note:** Detection rules are not shipped in the binary. They live in the
> separate `agent-reliability-rules` git repository
> (`https://github.com/trustabl/agent-reliability-rules`) and are resolved at scan
...
| **MCP** | Rust | ✅ dep-scan (`rmcp` in `Cargo.toml` / `Cargo.lock`) + file inventory | ◐ tools (`#[tool]`-attributed methods, official rmcp crate; name from the `name = "..."` arg or the method name, description from the `description = "..."` arg **or** the method's `///` doc comment, typed params). `#[tool]` is a real `attribute_item` node (no comment hack). Raw-string descriptions, `#[tool(name=...)]` on free functions outside an impl, and `#[prompt]` / resource shapes are gaps | ◐ field-based language:rust: MCP-021 (no description, doc-comment-aware), MCP-022 (ambiguous name). Body-fact rules need Rust AST predicate branches (fast-follow); untyped-params N/A (Rust is statically typed; the param schema lives in a separate `#[derive(JsonSchema)]` struct) |
| **Google ADK** | Python | ✅ dep-scan (`google-adk`) + file inventory | ✅ LlmAgent (+ Agent alias), SequentialAgent, ParallelAgent, LoopAgent, LanggraphAgent; FunctionTool wrapping; 13 built-in hosted tools; sub_agents edges | ✅ tool ADK-001..007, 009..012 (009 = print to stdout, 010 = subprocess, 011 = eval/exec/compile, 012 = SSRF); agent ADK-008, 101..108, 110..111; repo ADK-201 (SDK code without an agent-guidance doc: AGENTS.md/CLAUDE.md) |
...
| Skills | `SKILL.md` (basename, any depth: `.claude/skills/<name>/SKILL.md`, plugin `skills/`, nested). Parses `name`, `description`, `allowed-tools` (space-separated or YAML-list → `ToolGrants`), `argument-hint`, `disable-model-invocation`, plus **body facts** (dynamic-context exec commands, external URLs, prompt-injection markers incl. hidden-Unicode — zero-width, the Tags block U+E0000–E007F, and bidi overrides), a **bundled-file inventory** (script/markdown/binary/resource by extension; non-binary files are content-scanned, size-capped — scripts for network egress and credential reads, any file for committed secret literals), and `disallowed-tools` / `context` / `agent` (fork) / `HasHooks` / `user-invocable`. Audited by skill-scope rules: CSKILL-001 (auto-approved unrestricted shell), CSKILL-002/003 (dynamic-context execution + network/secret egress), CSKILL-010/011 (bundled-script egress + credential-read — the payload-in-aux-file surface body-only scanners miss), CSKILL-030 (hardcoded secret committed in a bundled file), CSKILL-020 (external-URL fetch), CSKILL-040 (injection markers incl. hidden-Unicode), CSKILL-050 (model-invocable + side-effecting tools), CSKILL-060 (description claims read-only but grants side-effecting tools — OWASP AST04 metadata mismatch). Skill rules carry no `language:` field (markdown, language-agnostic); the `claude_skill` pack loads unconditionally (skills are not an SDK import). Dependency manifests are parsed **repo-wide** (not just skill-bundled) into `ScanResult.Dependencies` — a deterministic supply-chain BOM across all six supported languages (`requirements.txt`/`pyproject.toml`/`Pipfile` pip, `package.json` npm, `go.mod` Go, `composer.json` Composer, `*.csproj` NuGet, `Cargo.toml` Cargo; `DepRef{name, version, ecosystem, source}`, sorted + deduped, **not** folded into `ScanID`), with an optional CycloneDX 1.5 export via `--bom-out`. Pure inventory by default: no network, no CVE matching (hands off to OSV-Scanner / Dependabot / syft). The opt-in `--vuln-scan` layer (`internal/vulndb`, TR-271) adds the CVE verdict — it matches each concretely-pinned dep against a pinned OSV snapshot (per-ecosystem PEP 440 / semver comparison + the OSV range algorithm, CVSS 3.x → severity) and emits both `ScanResult.Vulnerabilities` and synthesized advisory-ID findings that hit the normal exit codes and SARIF; the snapshot is fetched from osv.dev on first use, cached with an offline fallback (`trustabl vulndb pull` pre-warms it), and its version is folded into `ScanID` only when enabled. Declared ranges (no lockfile) are skipped, not guessed. See TR-278, TR-271. |
...
| Session config | `ClaudeAgentOptions(...)` constructor calls. Captures kwargs into a `KwargTree`; `permission_mode` is read by the repo-scope rule CSDK-202, `max_turns` absence by CSDK-204, and `disallowed_tools` absence (combined with an `acceptEdits` `permission_mode`) by CSDK-205 — `max_turns`/`disallowed_tools` absence share one predicate helper, `repoClaudeOptionsMissingKwarg`. Presence alone marks the repo `claude_agent_sdk` so the pack loads for options-only repos |
...
| Hosted tools | Closed set of 9 factories across `@openai/agents-core` and `@openai/agents-openai`: emits `HostedToolDef` with `SDK=openai_agents`, the canonical factory name, the options-object argument captured into `Kwargs` via `TSObjectKwargs` (e.g. `hostedMcpTool({allowedTools, requireApproval, ...})`; nil for a no-arg call like `fileSearchTool()`), and the factory call's own precise `Location` — carried through `ResolveEdges` via `HostedToolRef.Pending` rather than approximated to the agent's line |
...
| Built-in hosted tools | Closed set of 14 classes recognized as `HostedToolDef` with `SDK=google_adk`: `BashTool`, `GoogleSearchTool`, `VertexAiSearchTool`, `LangchainTool`, `CrewaiTool`, `AgentTool`, `LongRunningTool`, `LoadWebPage`, `ExitLoopTool`, `GoogleMapsGroundingTool`, `UrlContextTool`, `DiscoveryEngineSearchTool`, `EnterpriseSearchTool`, `MCPToolset` |
...
`interrupt_before` / `interrupt_after`, `store`) linked back onto the agent. The
graph's tools are resolved from its `ToolNode([...])` / `llm.bind_tools([...])`
call sites (`ResolveEdges`) — both inline list literals and the common
`tools = [...]; ToolNode(tools)` same-file variable form — and the dangerous
built-ins among them (`PythonREPLTool` / `ShellTool` / …) attach as
`HostedToolRefs`, so **agent rule LC-101 fires on a StateGraph that wires a
code-execution / shell tool** (`applies_to: [langchain_state_graph]`).
**Limitations:** a tools list passed cross-function / cross-module (not a
same-file list literal or `tools = [...]` variable) is not resolved; file-level
attribution gives every StateGraph in a file the file's graph tools (one graph
per file is the common case); and there is no loop-bound rule (`recursion_limit`
is an invoke-site config, not a constructor kwarg, so it is out of static reach).
...
| Native (built-in) tools | The dangerous subset `CodeExecutionTool` (code exec), `WebFetchTool` / `UrlContextTool` / `WebSearchTool` (model-chosen URL fetch). Wired in two shapes under the `capabilities=` / `builtin_tools=` kwargs (NOT the generic `tools=` list, so `ResolveEdges` scans those two kwargs specifically): the modern `capabilities=[NativeTool(CodeExecutionTool())]` wrapper (unwrapped one level to the inner class, including the inner constructor's kwargs so `WebFetchTool(force_download=True)` is visible) and the legacy `builtin_tools=[CodeExecutionTool()]` direct form → `HostedToolDef` (SDK `pydantic_ai`), consumed by agent rules PYD-102 / 103 / 104 |
| FileUrl `force_download` | File-level walk of `FileUrl` / `ImageUrl` / `AudioUrl` / `VideoUrl` / `DocumentUrl` constructors; the most-dangerous `force_download` (`"allow-local"` outranks `True`) is stamped on every Pydantic agent in that file as `AgentDef.FileURLForceDownload`. Consumed by PYD-104 via `agent_file_url_force_download` |

**v1 gaps:** the bare-function `tools=[fn]` ToolDef-synthesis shape (the agent
...
| **MCP cross-language follow-ups (Go/C#/PHP/Rust)** | **Go + C# + PHP + Rust MCP have all landed** — each via the same recipe: a dep needle, a tree-sitter pass over the existing smacker grammars (no new dependency), `#[…]`/attribute-based tool discovery, and field-based `language: <lang>` rules. **Go** (MCP-015/016): mark3labs `mcp.NewTool(...)` + official `mcp.AddTool(...)`; follow-ups: the official SDK's handler-struct param schema and metoro-io/mcp-golang's reflection-based `RegisterTool`. **C#** (MCP-017/018): `[McpServerTool]`; follow-ups: the `[McpServerTool(Name=...)]` override, variable-named `.csproj` needle, Semantic Kernel `[KernelFunction]` / AutoGen `[Function]`. **PHP** (MCP-019/020): `#[McpTool]` read from comment text (the grammar parses single-line attributes as comments); follow-ups: multi-line attributes, `#[McpResource]` / `#[McpPrompt]`, an untyped-params rule (type hints are optional). **Rust** (MCP-021/022): rmcp `#[tool]`, doc-comment-aware descriptions; follow-ups: raw-string descriptions and `#[prompt]`/resource shapes. **Shared follow-ups across all four:** body-fact rules (shell/SSRF/timeout) need per-language AST predicate branches (currently TS/JS-only), and MCP resource/prompt registrations are not yet discovered in any language. **Rust** is the last mainstream MCP language; **Java / Ruby / Kotlin** MCP servers exist but have no needle or parser pass yet |
...
| **LangChain class tools + raw `StateGraph` + TS provider hosted tools** | Three additive discovery gaps in the LangChain pack: `class X(BaseTool)` / `extends StructuredTool` (a class shape, not a call); the raw `StateGraph` → `.compile()` graph agent — **Python is fully landed** (`langgraph_graph.go`: discovered + import-bound, its `ToolNode` / `bind_tools` tools resolved, and LC-101 fires on it via `langchain_state_graph`; remaining is the TS equivalent and cross-function tool-list resolution); and the date-stamped TS provider hosted tools (`shell()` / `bash_*` / `applyPatch`). Each is discovery-only, no schema change |
...
5. **LangChain / LangGraph pack** has **landed** (newest SDK row): 15 rules
   ...
   file. The raw `StateGraph` → `.compile()` graph agent is **fully landed** in
   Python (`langgraph_graph.go`: discovered + import-bound, its `ToolNode` /
   `bind_tools` tools resolved, and LC-101 firing via `langchain_state_graph`);
   the TS equivalent is pending. Remaining
   (tracked in the gaps table): class-based tools (`class X(BaseTool)`
...
6. **Four SDK rows landed 2026-06-05** — CrewAI, AutoGen / AG2, and Pydantic AI
   ...
   engine binary and a rule pack in `agent-reliability-rules`: CrewAI CREW-001..006 / 101..108
   ...
   `.js` parsing (the VAI-011 TS-timeout predicate has shipped); Pydantic
   bare-`tools=[fn]` synthesis + the `RunContext` param-strip for PYD-002
   (PYD-104 `force_download` shipped).
